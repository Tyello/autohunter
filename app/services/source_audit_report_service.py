...
def write_reports(samples: list[dict[str, Any]], out_dir: Path) -> dict[str, Path]:
    ...
    lines += [
        ...
        "- Obrigatórios: source, external_id, url, title, price.",
        "- Opcionais desejáveis: location/city/state, year, mileage_km, thumbnail_url/images, fuel_type, transmission, make, model, version, seller_type, color.",
        "- Dependem de detalhe em várias fontes: description, seller_type, risk_signals.",
        "- Flags críticas quando ausentes: missing_url, missing_external_id, missing_price.",
        "- Não tratar como missing_* global em todas as fontes: seller_type, description, risk_signals.",
    ]
    md_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
