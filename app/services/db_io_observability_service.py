...
def collect_db_io_metrics(db: Session) -> dict[str, Any]:
    ...
    notification_age = db.execute(text("select extract(epoch from (now() - min(created_at)))::int from notifications where status in ('queued','processing')")).scalar() or 0
    scrape_job_age = db.execute(text("select extract(epoch from (now() - min(created_at)))::int from scrape_jobs where status in ('queued','running')")).scalar() or 0
    churn_estimate = sum(int(v or 0) for v in recent_rows.values()) + sender_backlog + sum(int(v or 0) for v in scrape_jobs.values())
    return {"top_tables": sorted(table_rows.items(), key=lambda x: x[1], reverse=True), "recent_24h": sorted(recent_rows.items(), key=lambda x: x[1], reverse=True), "scrape_jobs_by_status": scrape_jobs, "notifications_by_status": notifications, "sender_backlog": sender_backlog, "notification_backlog_oldest_age_seconds": int(notification_age or 0), "scrape_job_backlog_oldest_age_seconds": int(scrape_job_age or 0), "source_runs_24h": source_runs_24h, "approx_io_churn": churn_estimate}
...
def render_db_io_metrics(data: dict[str, Any]) -> str:
    ...
    return "\n".join([
        ...
        f"Idade backlog notifications: {data.get('notification_backlog_oldest_age_seconds', 0)}s",
        f"Idade backlog scrape_jobs: {data.get('scrape_job_backlog_oldest_age_seconds', 0)}s",
