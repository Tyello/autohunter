...
def _field_match(listing: CarListing, field: str, op: str, val: str) -> bool:
    if field == "state":
        ...
        current = normalize_body_type(getattr(listing, "body_type", None) or "")
        target = normalize_body_type(val)
    else:
        return True
    ...
    for f in filters:
        ...
        if field in {"color", "city", "state", "seller_type", "body_type"}:
            ...
            if not _field_match(listing, field, op, val):
                ...
                if field == "body_type":
                    body_type = normalize_body_type(getattr(listing, "body_type", None) or "")
                    if not body_type:
                        return "filter_body_type_missing"
                    return "filter_body_type_eq" if op == "eq" else "filter_body_type_neq"
