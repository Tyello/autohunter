...
async def _admin_health(update: Update, raw_args: Optional[List[str]] = None):
    _admin_health_module.is_admin = is_admin
    _admin_health_module.sanitize_for_telegram = sanitize_for_telegram
    _admin_health_module._reply_chunked = _reply_chunked
    _admin_health_module.list_sources = list_sources
    return await admin_health(update, raw_args)
...
async def _admin_audit(update: Update, raw_args: Optional[List[str]] = None):
    ...
    _admin_health_module.sanitize_for_telegram = sanitize_for_telegram
    _admin_health_module._reply_chunked = _reply_chunked
    _admin_health_module.collect_operational_alerts = collect_operational_alerts
    return await admin_audit(update, raw_args)


async def _admin_errors(update: Update, raw_args: List[str]):
    _admin_health_module.sanitize_for_telegram = sanitize_for_telegram
    return await admin_errors(update, raw_args)
