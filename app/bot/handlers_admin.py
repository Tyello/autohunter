...
async def _admin_fb_sessions(update: Update):
    ...
    try:
        ...
        stale_active = (
            ...
            .filter(FBSession.status == "ACTIVE", FBSession.last_ok_at.is_not(None), FBSession.last_ok_at < stale_cutoff)
            .scalar()
            or 0
        )
        top_users = (
        ...
        users_text = ", ".join([f"{u}:{c}" for u, c in top_users]) if top_users else "-"

        await update.message.reply_text(
            "FB sessions\n"
            f"by_status: {status_text}\n"
            f"top_errors: {error_text}\n"
            f"stale_active_gt_7d: {stale_active}\n"
            f"top_users_recurrent_errors: {users_text}\n"
