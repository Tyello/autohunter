...
| `app/notifications/email.py`, `app/notifications/whatsapp.py`, `app/notifications/webhook.py`, `app/notifications/manager.py` | Removido no P1-B (2026-05-21) | Código paralelo sem uso no runtime Telegram atual. | Baixo | Mantido fora do runtime; reintroduzir apenas com caso operacional explícito. |
| Documentos temporários antigos (`PATCH_*`, handoffs, propostas de preço/fontes/UX antigas) | Removido/dispensável | Conteúdo histórico/migração/planejamento divergia ou competia com docs vivos. | Baixo código / Médio onboarding | Manter fora do repo; usar `docs/README.md`, `docs/ROADMAP.md`, `docs/USER_FLOWS.md` e docs donos por frente. |
