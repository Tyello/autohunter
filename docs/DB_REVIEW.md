...
## Update 2026-08-10 — causa raiz do IO ainda alto apesar dos índices P0

Investigação de continuidade do IO budget (`seq_tup_read` desproporcional em `system_logs`/`source_runs`, 5ª tabela suspeita). Achado principal:

- **Bug real, não suspeita:** `scripts/cleanup_operational_data.py` roda via cron a cada 6h (`config/raspberry-pi/crontab`) com `--apply`, mas desde a migration `5c8f1a2b3d4e_core_data_delete_guardrails.py` (2026-05-29) as tabelas `notifications` e `wishlist_listing_activity` têm trigger `BEFORE DELETE`/`BEFORE TRUNCATE` que bloqueia DELETE sem `SET LOCAL app.allow_core_data_delete = 'on'`. O script nunca setava essa flag e não isolava falhas por regra, então a primeira exceção (na regra `notifications`) abortava o `main()` inteiro — `wishlist_listing_activity` nunca era limpo. Isso ficou quebrado silenciosamente por ~2,5 meses (todo cron desde 2026-05-29), explicando o crescimento contínuo dessas duas tabelas e é a explicação mais provável para a "5ª tabela" com IO desproporcional.
- **Corrigido:** `scripts/cleanup_operational_data.py` agora seta break-glass só para as duas tabelas protegidas e isola cada regra em `try/except` (uma falha não impede as demais). Lógica das tabelas não-protegidas extraída para `app/services/operational_data_cleanup_service.py`, reusada por um novo job do APScheduler (`app/scheduler/operational_data_cleanup_job.py`, id `operational_data_cleanup`, `interval hours=6`, jobstore persistente `SQLAlchemyJobStore`, mesmo padrão do job mensal da FIPE). O cron continua ativo, agora estritamente necessário só para `notifications`/`wishlist_listing_activity`.
- **Índice novo:** `ix_system_logs_component_message_created_at` (migration `b2c4d6e8f0a1_system_logs_component_message_index.py`) — cobre `admin_handlers_health.py`'s lookup `component='scheduler' AND message='heartbeat' ORDER BY created_at DESC LIMIT 1`, que antes fazia heap-fetch sem índice em `message`.
- **`scrape_jobs` com 11,7M `idx_scan`:** esperado, não é bug. HTTP workers fazem polling a cada 2s (2 workers), browser worker a cada 5s, e `requeue_stale_running_jobs` roda em toda invocação — isso sozinho já soma ordens de milhão de índex-scans por mês. Não requer ação.
- **`source_configs` com 566k seq_scan / 0 idx_scan em tabela de 48kB:** aceitável — tabela de config pequena (~dúzias de linhas), lida via cache TTL de 60s (`source_configs_service.py`), mas alguns caminhos (`ensure_source_configs`, `list_source_config_snapshots` em cache-miss) fazem `SELECT *` sem filtro. Não é candidata a retenção (não é log/evento).
- **Janelas de retenção padronizadas para 10 dias** nas tabelas puramente operacionais/efêmeras: `operational_retention_system_logs_days` (7→10), `operational_retention_telemetry_events_days` (7→10), `operational_retention_source_runs_days` (30→10). `notifications` (90d) e `wishlist_listing_activity` (90d) mantidos — têm valor de auditoria/relatório de entrega (`sent_at`/digest semanal), não são puramente descartáveis. `scrape_jobs` mantido em escala de horas (fila, não log).
- **Pendência sem acesso a produção:** confirmar via `pg_stat_reset()`/`stats_reset` se os 47B de `seq_tup_read` são débito histórico acumulado desde antes dos índices P0, ou se persistem após um período de observação pós-fix. Rodar a query de validação da seção anterior novamente após ~48h deste deploy.
- **Nota separada, fora de escopo:** o repositório já tem múltiplos heads divergentes no Alembic (`p0_02_fipe_update_runs`, `p1_notif_dq_partial`, e agora `b2c4d6e8f0a1` no lugar de `p0_03_supabase_io_indexes`) — precisa de uma migration de merge dedicada antes do próximo `alembic upgrade head` em produção.
