...
| `mega_auctions` | `experimental` | enrich de detalhe ativo com extração defensiva (lances, datas, status, imagem e city/UF) e diagnóstico no inspect; cobertura ainda deve ser observada por ciclos | manter fora de `user_eligible`; monitorar por `/admin auctions monitor mega` e `/admin auctions source-history mega` |
