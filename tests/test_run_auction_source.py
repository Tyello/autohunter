...
def test_invalid_source_raises_clear_error():
    try:
        ...
        assert "win_auctions" in str(exc)
    else:
...
def test_run_dry_run_win(monkeypatch):
    monkeypatch.setattr("scripts.run_auction_source.fetch_win_lots", lambda limit: [NormalizedAuctionLot(source="win_auctions", external_id="w1")])
    run(source="win_auctions", limit=1, dry_run=True)
