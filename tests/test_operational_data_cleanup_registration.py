...
def test_start_scheduler_registers_operational_data_cleanup_job(monkeypatch):
    ...
    jobs = [j for j in sched.jobs if j.get("id") == "operational_data_cleanup"]
    assert len(jobs) == 1
    assert jobs[0]["trigger"] == "interval"
    assert jobs[0]["hours"] == 6
    assert jobs[0]["replace_existing"] is True
