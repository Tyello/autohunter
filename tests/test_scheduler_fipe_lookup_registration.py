...
def test_start_scheduler_registers_fipe_lookup_job(monkeypatch):
    ...
    jobs = [j for j in sched.jobs if j.get("id") == "fipe_lookup_poll"]
    assert len(jobs) == 1
...
def test_fipe_lookup_job_uses_dedicated_executor(monkeypatch):
    from app.scheduler import run as run_mod
    ...
    assert "fipe_lookup" in sched.init_kwargs.get("executors", {})

    jobs = [j for j in sched.jobs if j.get("id") == "fipe_lookup_poll"]
    assert len(jobs) == 1
    assert jobs[0]["executor"] == "fipe_lookup"
