...
def test_admin_audit_guard(monkeypatch):
    ...
    up = _Update(999)
    asyncio.run(health_module.admin_audit(up, []))
...
def test_admin_audit_has_sections(monkeypatch, db):
    ...
    monkeypatch.setattr(health_module, "_reply_chunked", _fake_chunked)
    asyncio.run(health_module.admin_audit(_Update(1), []))
    txt = cap["t"]
...
def test_admin_audit_status_ok(monkeypatch, db):
    monkeypatch.setattr(health_module, "is_admin", lambda _cid: True)
    monkeypatch.setattr(health_module, "sanitize_for_telegram", lambda t: t)
    monkeypatch.setattr(health_module, "collect_operational_alerts", lambda _db, now=None, consume_cooldown=False: [])
