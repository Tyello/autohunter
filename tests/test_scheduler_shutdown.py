...
def test_interpreter_shutdown_error_not_marked_failed(monkeypatch):
    from app.scheduler import http_queue_job

    shutdown_state.request_shutdown("SIGTERM")
    calls = {"failed": 0}
    ...
    monkeypatch.setattr(http_queue_job, "SessionLocal", lambda: DummyDB())
    monkeypatch.setattr(http_queue_job, "dequeue_next_job", lambda *_a, **_k: None)
    monkeypatch.setattr(http_queue_job, "mark_failed", lambda *_a, **_k: calls.__setitem__("failed", calls["failed"] + 1))
